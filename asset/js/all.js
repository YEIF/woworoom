...
function addOrder (e) {
  ...
  if (errorMessage) {
    Object.keys(errorMessage).forEach(function (keys) {
      const messageSeletor = document.querySelector(`.orderInfo-inputWrap [data-message=${keys}]`)
...
function renderCarts (carts, finalTotal) {
  let str = ''
  if (carts.length === 0) {
    // console.log("沒有商品")
    ...
    carts.forEach(element => {
      const money = Number(element.product.price) * Number(element.quantity)
      const type = element.quantity === 1 ? 'disable' : ''
      str += `                    
        <tr>
            <td>
...
function formListener (e) {
  ...
  if (errorMessage) {
    ...
    if (filters.length > 0) {
      const messageSeletor = document.querySelector(`.orderInfo-inputWrap [data-message=${filters}]`)
      messageSeletor.textContent = errorMessage[filters]
      messageSeletor.setAttribute('style', 'display:block;')
    } else if (filters.length === 0) {
      const messageSeletor = document.querySelector(`.orderInfo-inputWrap [data-message=${e.target.name}]`)
...
function productsListListener (e) {
  ...
  if (productId) {
    const carts = cartsData.filter(item => item.product.id === productId)
    if (carts.length !== 0) {
      Swal.fire({
        position: 'center',
        icon: 'info',
...
function shoppingCartListener (e) {
  ...
  switch (value) {
    case 'js-deleteCartAll':
      ...
      break
    case 'js-delProduct':
      cartsId = e.target.getAttribute('data-del')
      Swal.fire({
        position: 'center',
        icon: 'success',
        title: '刪除商品成功',
      ...
      deleteProduct(cartsId)
      break
    case 'js-patchAdd':
      cartsId = e.target.getAttribute('data-add')
      num = e.target.getAttribute('data-num')
      patchProduct(cartsId, (parseInt(num)) + 1)
      Swal.fire({
        position: 'center',
        icon: 'success',
        title: '增加數量成功',
      ...
      break
    case 'js-patchRemove':
      cartsId = e.target.getAttribute('data-remove')
      num = e.target.getAttribute('data-num')
      patchProduct(cartsId, (parseInt(num)) - 1)
      Swal.fire({
        position: 'center',
    ...
    case 'js-patchproduct': {
      ...
      patchSelector.addEventListener('change', function (e) {
        ...
        if (patchNum <= 0 || isNaN(patchNum)) {
          ...
          patchProduct(cartsId, patchNum)
        }
        // console.log(`更改${cartsId}訂單，數量為${e.target.value}`);
      })
      break
    }
    default:
...
function filterProductList (e) {
  const filterData = productsData.filter(item => e.target.value === item.category || e.target.value === '全部')
